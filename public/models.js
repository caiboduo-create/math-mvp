(function () {
  ...
  function normalizeSignedSemanticQuestion(result) {
    ...
    return {
      ...
      steps,
      explanation: `${result.explanation || ""}${result.explanation ? " " : ""}${semanticExplanation}`
    };
  }
  ...
  function generateQuestion(modelId) {
    ...
    switch (modelId) {
      case "rational-number": {
        ...
        const start = randomInt(6, 24);
        const change = randomInt(2, 18);
        const value = -start + change;
        const question = scenario === "temperature"
          ? `气温原来是 -${start}℃，后来上升 ${change}℃。现在气温是多少？`
          : scenario === "water"
            ? `潜水员在水下 ${start} 米处，随后上浮 ${change} 米。用水面为 0 米表示，现在的位置是多少？`
            : `某地低于海平面 ${start} 米，后来上升 ${change} 米。以海平面为 0 米，现在的位置是多少？`;
        return questionResult(
          question,
          `${value}`,
          {
            answerValue: value,
            tolerance: 0.01,
            steps: [`把起点记作 -${start}`, `变化记作 +${change}，列式 -${start} + ${change} = ${value}`],
            explanation: "正负数应用题要先算数值结果，再把符号翻译成生活语义。"
          }
        );
      }
  ...
  const customInteractiveTemplates = {
    "rational-number": "rational-number-lab",
    "polynomial-add-sub": "like-term-sorter",
    "linear-equation-one": "equation-step-balance",
    "basic-geometry": "geometry-basics-builder",
    "real-number": "real-number-classifier",
    "linear-equations-two": "system-elimination-lab",
    inequality: "inequality-number-line",
    "data-collection": "survey-chart-lab",
    "congruent-triangle": "congruence-test-lab",
    symmetry: "symmetry-mirror-lab",
    "isosceles-triangle": "isosceles-triangle-lab",
    "polynomial-multiply": "polynomial-area-multiply",
    "multiplication-formula": "multiplication-formula-area",
    factorization: "factorization-puzzle",
    "fraction-expression": "fraction-expression-lab",
    "quadratic-radical": "radical-simplifier",
    parallelogram: "parallelogram-shear-lab",
    rectangle: "rectangle-measure-lab",
    rhombus: "rhombus-diagonal-lab",
    square: "square-properties-lab",
    "data-analysis": "data-statistics-lab",
    "quadratic-equation": "quadratic-root-lab",
    "inverse-function": "inverse-variation-lab",
    "similar-triangle": "similarity-scale-lab",
    trigonometry: "right-triangle-ratio-lab",
    "projection-view": "three-view-lab",
    probability: "probability-simulator-lab"
  };

  const interactiveDescriptions = {
    "rational-number-lab": "在数轴上移动有理数，观察比较、绝对值和加减法过程。",
    "equation-balance": "拖动未知数取值，观察方程两边是否平衡。",
    "equation-step-balance": "通过等式天平，分步理解两边同时操作的解方程过程。",
    "like-term-sorter": "把整式中的项自动归类，观察同类项为什么能合并。",
    "algebra-tiles": "用代数块观察表达式的结构。",
    "geometry-basics-builder": "切换直线、射线、线段和角，观察端点、长度和角度类型。",
    "real-number-classifier": "把数字卡片归入集合，理解有理数、无理数和实数的关系。",
    "system-elimination-lab": "分步演示二元一次方程组的加减消元、代入消元和交点意义。",
    "inequality-number-line": "分步骤解不等式，并在数轴上观察解集和不等号方向变化。",
    "survey-chart-lab": "调整调查数据，同步生成统计表、条形图和扇形图。",
    "congruence-test-lab": "选择判定条件，观察两个三角形何时能确定全等。",
    "symmetry-mirror-lab": "切换对称轴，观察原图形和对称图形的距离关系。",
    "isosceles-triangle-lab": "拖动顶点，观察等腰三角形的底角和三线合一。",
    "polynomial-area-multiply": "用矩形面积模型分步理解整式乘法。",
    "multiplication-formula-area": "用拼图面积解释平方差和完全平方公式。",
    "factorization-puzzle": "从系数条件和面积拼图反向得到因式分解。",
    "fraction-expression-lab": "分步骤观察分式的约分、通分和加减。",
    "radical-simplifier": "选择完全平方因数，分步化简二次根式并比较近似值。",
    "parallelogram-shear-lab": "拖动倾斜程度并剪拼成长方形，理解底乘高。",
    "rectangle-measure-lab": "调整长和宽，比较面积、周长和对角线变化。",
    "rhombus-diagonal-lab": "调整对角线，观察菱形面积和对角线性质。",
    "square-properties-lab": "调整边长并分类正方形继承的几何性质。",
    "data-statistics-lab": "修改数据并加入异常值，观察统计量变化。",
    "quadratic-root-lab": "用因式分解、判别式和数轴理解一元二次方程根。",
    "inverse-variation-lab": "调整 k 和 x，观察反比例函数和象限变化。",
    "similarity-scale-lab": "拖动相似比，观察对应边、周长比和面积比。",
    "right-triangle-ratio-lab": "拖动锐角，实时计算正弦、余弦和正切。",
    "three-view-lab": "切换观察方向，匹配立体模型的三视图。",
    "probability-simulator-lab": "选择摸球、转盘或硬币实验，比较理论概率和实验频率。"
  };
